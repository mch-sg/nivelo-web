...
?>
<title><?php if($host !== 'devmch.online/chat_room_s' && isset($chat_room_name)) { echo htmlspecialchars($chat_room_name, ENT_QUOTES); } else { echo "Chatrum"; } ?> - Nivelo</title>
...
$host = $_SERVER['SERVER_NAME']  . $_SERVER['REQUEST_URI'];
if($_SESSION['useruid'] == $user_from_id || $_SESSION['useruid'] == $user_to_id && $host !== 'devmch.online/chat_room_s'){
    echo "<br><br>
...
} else if ($_SESSION['useruid'] != $user_from_id || $_SESSION['useruid'] != $user_to_id && $host !== 'devmch.online/chat_room_s') {
    echo "<br><br>
    <div class='main-content' style='position: relative;'>
...
if($host == 'devmch.online/chat_room_s'){
    echo "<br><br>
    <div class='main-content' style='position: relative;'>
    <div class='chatbox-container chat-scale' style='margin-bottom: 0;bottom: 45px;'>"; // ! Lånt chat-scale css
    echo "<div class='chatbox' id='chatbox' style='font-weight: 200;color:white; white-space: normal; overflow: auto; word-wrap: break-word;'>";
...
else if (!$authorized && $host != 'devmch.online/chat_room_s') {
    die("Du har ikke adgang til denne chat.");
}

// Udprinter messages fra prev. LOAD
if(count($rows) > 0 && $host != 'devmch.online/chat_room_s') {
    foreach($rows as $row) {
...
} else if(count($rows) == 0 && $host != 'devmch.online/chat_room_s') {
